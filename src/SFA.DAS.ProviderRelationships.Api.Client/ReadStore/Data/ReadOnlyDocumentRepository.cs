...
namespace SFA.DAS.ProviderRelationships.Api.Client.ReadStore.Data
{
    public abstract class ReadOnlyDocumentRepository<TDocument> : IReadOnlyDocumentRepository<TDocument>
    {
        ...
        public virtual async Task<TDocument> GetById(Guid id, RequestOptions requestOptions = null, CancellationToken cancellationToken = default)
        {
            try
            {
                var response = await _documentClient
                    .ReadDocumentAsync<TDocument>(UriFactory.CreateDocumentUri(_databaseName, _collectionName, id.ToString()), requestOptions, cancellationToken).ConfigureAwait(false);
