...
public class ClientOutboxPersisterV2 : IClientOutboxStorageV2
{
    ...
    public async Task<IClientOutboxTransaction> BeginTransactionAsync()
    {
        ...
        DbTransaction transaction = await obj.BeginTransactionAsync();
    ...
    public async Task<ClientOutboxMessageV2> GetAsync(Guid messageId, SynchronizedStorageSession synchronizedStorageSession)
    {
        ...
        await using DbCommand command = sqlStorageSession.Connection.CreateCommand();
        ...
        await using DbDataReader reader = await command.ExecuteReaderAsync(CommandBehavior.SingleRow).ConfigureAwait(continueOnCapturedContext: false);
    ...
    {
        await using DbConnection connection = await _connectionBuilder.OpenConnectionAsync().ConfigureAwait(continueOnCapturedContext: false);
    ...
    {
        ISqlStorageSession sqlStorageSession = synchronizedStorageSession.GetSqlStorageSession();
        await using DbCommand dbCommand = sqlStorageSession.Connection.CreateCommand();
