...
using ConfigurationKeys = SFA.DAS.PAS.Account.Api.ClientV2.Configuration.ConfigurationKeys;
...
public static class AddConfigurationOptionsExtension
{
        ...
        IConfiguration configuration)
    {
        ...
        services.Configure<ProviderRelationshipsApiConfiguration>(configuration.GetSection($"{Configuration.ConfigurationKeys.ProviderRelationships}:ProviderRelationshipsApi"));
        ...
        services.Configure<RecruitApiConfiguration>(configuration.GetSection($"{Configuration.ConfigurationKeys.ProviderRelationships}:RecruitApiClientConfiguration"));
        services.AddSingleton(cfg => cfg.GetService<IOptions<RecruitApiConfiguration>>().Value);

        services.Configure<RegistrationApiConfiguration>(configuration.GetSection($"{Configuration.ConfigurationKeys.ProviderRelationships}:RegistrationApiClientConfiguration"));
        services.AddSingleton<IRegistrationApiConfiguration>(cfg => cfg.GetService<IOptions<RegistrationApiConfiguration>>().Value);

        services.Configure<RoatpApiConfiguration>(configuration.GetSection($"{Configuration.ConfigurationKeys.ProviderRelationships}:RoatpApiClientSettings"));
        services.AddSingleton(cfg => cfg.GetService<IOptions<RoatpApiConfiguration>>().Value);
